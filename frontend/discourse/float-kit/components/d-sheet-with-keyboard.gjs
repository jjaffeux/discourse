...
class Root extends Component {
   ...
   * Following Silk's actual implementation: always use "bottom" placement
   * (The React example in Silk docs shows ["top", "bottom"] but the actual
   * rendered HTML uses bottom sheet layout on all viewports)
   */
   ...
   * Following Silk's actual implementation: always use "bottom" track
   */
  get tracks() {
    return "bottom";
  }
   ...
   *
   * Note: View-level props (placement, tracks, swipeOvershoot, nativeEdgeSwipePrevention, onClickOutside)
 ...
 * - DScrollContent (c3) - the actual scrollable content
 *
...
const ScrollView = <template>
  <DScrollRoot class="SheetWithKeyboard-scrollView" ...attributes as |scroll|>
    <scroll.View
