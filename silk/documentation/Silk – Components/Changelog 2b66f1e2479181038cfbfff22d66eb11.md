...
- Make `theme-color` HTML meta tag manipulations safer in contexts where it cannot be defined. https://github.com/silk-hq/silk/issues/18

# v0.9.12

**Release date**: 2025-09-12

- Change the behavior of the `onClickOutside` event handler on `<Sheet.View>` so it runs even if propagation is stopped on `click` events fired on `<Sheet.View>`. https://github.com/silk-hq/silk/issues/100
- Expand the  `componentRef` prop type on `<Scroll.Root>` so it accept `null`.
...
- Change the way Silk low-level CSS styles are imported. Follow the [migration guide](Silk%20%E2%80%93%20Migrating%20from%20v0%208%20x%20to%20v0%209%20x%202b66f1e24791813ba912f5086dd83892.md) on how to import them.
