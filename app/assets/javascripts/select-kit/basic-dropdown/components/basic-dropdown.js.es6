...
export default Ember.Component.extend({
  tagName: "",
  layoutName: "select-kit/basic-dropdown/templates/components/basic-dropdown",
  top: null,
  left: null,
  right: null,
  width: null,
  ...
  init() {
    ...
    this.setProperties({ otherStyles: {}, publicAPI: {} });
    ...
    this.dropdownId =
      this.dropdownId || `ember-basic-dropdown-content-${publicAPI.uniqueId}`;
    if (this.onInit) {
  ...
  didReceiveAttrs() {
    ...
    if (newDisabled && !oldDisabled) {
      Ember.join(this, this.disable);
    } else if (!newDisabled && oldDisabled) {
      Ember.join(this, this.enable);
    }
  },
  ...
  reposition() {
    ...
    this.destinationElement =
      this.destinationElement || document.getElementById(this.destination);
    ...
    let positionData = (this.calculatePosition || calculatePosition)(
      triggerElement,
      dropdownElement,
      this.destinationElement,
      options
  ...
  updateState(changes) {
    let newState = Ember.set(
      this,
      "publicAPI",
      Object.assign({}, this.publicAPI, changes)
    );
    if (this.registerAPI) {
