...
function dropdownIsValidParent(el, dropdownId) {
  ...
  if (closestDropdown) {
    let trigger = document.querySelector(
      `[aria-owns=${closestDropdown.attributes.id.value}]`
    );
...
export default Ember.Component.extend({
  tagName: "",
  layoutName: "select-kit/basic-dropdown/templates/components/basic-dropdown-content",
  ...
  didInsertElement() {
    ...
    this.send("setup",  dropdownContainer);
    this.dropdown.actions.reposition();
    this.send("setupMutationObserver", dropdownContainer);
    this.send("animateIn", dropdownContainer);
  },
  ...
  willDestroyElement() {
    ...
    this.send("teardownMutationObserver");
    this.send("animateOut", dropdownContainer);
    this.send("teardown");
  },
  ...
  actions: {
    setup(dropdownElement) {
      let triggerElement = document.querySelector(
        `[data-ebd-id=${this.dropdown.uniqueId}-trigger]`
      );
    ...
    if (this.preventScroll === true) {
      let wheelHandler = event => {
        if (
          ...
          const availableScroll = getAvailableScroll(
            event.target,
            dropdownElement
          );
